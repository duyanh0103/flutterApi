...
//SAMPLE DATA
const c_data_sample = require('../samples/category')

router.get('/init', (req, res, next) => {
    try {
        Category.insertMany(c_data_sample.getSampleData)
    }
    catch (err) {
        console.log(err)
...
router.get('/:sn', (req, res, next) => {
    ...
    Product.find({ category_sn: sn })
        .then(result => {
            ...
            return res.json({ code: 0, message: 'Lấy dữ liệu product category thành công', data: result })
        })
        .catch(err => {
            return res.json({ code: 100, message: err })
        })
